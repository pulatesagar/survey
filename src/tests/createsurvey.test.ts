...
describe('Create Survey Scenarios', () => {
  ...
  test("Create Survey", async () => {
    ...
    for (i = 1; i <= 10; i++) {
      ...
      var surveyDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate() + i);
      ...
      // Enter Survey Name
      await page.type(surveyName, surveyNameValue + " " + i);
      ...
      await page.selectOption(surveyAccessabilityOption, surveyAccessabilityOptionValue + (index == 0 ? 4 : index));
      ...
      expect(createdSurvey).toBe(surveyNameValue + " " + i);
