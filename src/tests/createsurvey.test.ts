...
beforeAll(async () => {
  browser = await chromium.launch({ headless: false , devtools:true,slowMo:500});
...
describe('Create Survey Scenarios', () => {
  test.each([[1], [2], [3], [4], [5]])
    ...
    await page.fill(s_survey_name, s_survey_name_value + " " + a);
    await page.fill(s_survey_expiry, finalSurveyDate);
    await page.fill(s_survey_url, s_survey_url_value);
    await page.fill(s_survey_from_email_address, s_survey_from_email_address_value);
